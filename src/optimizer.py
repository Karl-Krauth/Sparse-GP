...
def optimize_model(model, optimization_config, max_iterations=200, mog_threshold=1e-4,
                   objective_threshold=1e-5):
    """
    Optimise the model by alternating the optimization through sets of parameters.
    ...
    try:
        while (not checker.is_converged() and
            ...
            checker.update_parameters(model.objective_function(), *model.get_gaussian_mixture_params())
            current_iter += 1
...
def bfgs(model, max_fun=None, apply_bounds=False):
    ...
    for num_tries in xrange(4):
        try:
            fmin_l_bfgs_b(func=wrapper.objective_function,
                          x0=starting_params,
...
class ModelWrapper(object):
    ...
    def update(self, new_params):
        ...
        except (ValueError, JitChol) as exception:
    ...
    def objective_function_gradients(self, new_params):
        self.update(new_params)
        return self._model.objective_function_gradients().copy()
