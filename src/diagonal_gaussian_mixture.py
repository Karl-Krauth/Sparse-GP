...
class DiagonalGaussianMixture(gaussian_mixture.GaussianMixture):
    ...
    def __init__(self, num_components, num_latent, initial_mean):
        ...
        self.invC_klj_Sk = np.empty((self.num_components, self.num_components, self.num_latent, self.num_dim))
        self.covars = np.random.uniform(low=0.5, high=0.5, size=(self.num_components, self.num_latent, self.num_dim))
